...
"""
Main class window to show live graph of load cell data during various verification tests
"""
class MainWindow(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Verification Testing")
        self.geometry("400x300")
        self.saved_path = tk.StringVar()

        # Create a button to open the live graph window
        self.open_live_btn = tk.Button(self, text="Start Test", 
                                       command= lambda: self.open_live_window(test_type="sine"), 
                                       width=20, height=2)
        self.open_live_btn.pack(pady=50)

    def open_live_window(self, test_type):
        """ Open the live graph window and hide the main window """
        self.withdraw()  # Hide main window
        self.live_window = LiveWindow(self, self, test_type=test_type)

    def _end_testing(self):
        """ Show main window again after closing live graph window """
        self.deiconify()  # Show main window again
"""
Class to show live window to display load cell data during various verification tests
"""
class LiveWindow(tk.Toplevel):
    def __init__(self, parent, main_window, test_type):
        ...
        if test_type == "sine":
            self.anim = FuncAnimation(self.fig, self.update_plot_sine, interval=50, blit=False)
        
        # On close of window behavior
        self.protocol("WM_DELETE_WINDOW", self.on_close)


    def update_plot_sine(self, frame):
        """ 
    ...
    def save(self):
        ...
        self.fig.savefig(f'test/{save_path.parent}/LoadCellPlot.png')

        date = datetime.now()
        am_pm = date.strftime('%p')
        filename = f"test/Live Graph_{date.month}-{date.day}-{date.year}_{date.hour}-{date.minute}-{date.second}-{am_pm}.xlsx"
        workbook = xlsxwriter.Workbook(save_path / filename)
        worksheet = workbook.add_worksheet(f"LoadCellData")
